...
reader = csv.reader(open('ccy_code.csv', 'r'))
d = {}
for row in reader:
   k, v = row
   d[k] = v
...
def parse_price(price):
    ...
    if not p:
        return 0
    if len(price) > 3 and (p[0:3] in d):
        fx_rate = float(d[p[0:3]])
        abc = str(int(int(locale.atoi(re.sub('[^0-9,]', "", p))) * fx_rate))
        return abc
    return locale.atoi(re.sub('[^0-9,]', "", p))
...
def parse_duration(duration_string):
    ...
    if "min" in duration_string:
        if "h" in duration_string: # eg: "2h 49min"
            ...
            return int(duration_string.replace(',', '').split('min')[0])
    else:
